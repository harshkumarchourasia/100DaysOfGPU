...
{
    float *d_data, *d_sums, *d_result, *d_transpose; //, *transpose;
    //transpose = (float*)malloc(nx *ny * sizeof(float));
    ...
    dim3 grid_dim((ny + BLOCK_DIM_2D - 1) / BLOCK_DIM_2D, (nx + BLOCK_DIM_2D - 1) / BLOCK_DIM_2D);
    dim3 block_dim(BLOCK_DIM_2D, BLOCK_DIM_2D);
    mean_center_and_transpose<<<grid_dim, block_dim>>>(d_data, ny, nx, d_transpose, d_sums);
    
    /*
    gpuErrchk(cudaMemcpy(transpose, d_transpose, nx * ny * sizeof(float), cudaMemcpyDeviceToHost));
    printf("\nTranspose:\n");
    for(int i = 0; i < nx; i++){
        for(int j = 0; j<ny;j++){
            printf("%f ", transpose[i*ny+j]);
        }
        printf("\n");
    }*/
    ...
    float *_result = (float*)malloc(ny*ny*sizeof(float));
